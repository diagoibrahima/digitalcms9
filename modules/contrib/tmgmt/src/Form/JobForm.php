  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($job->isSubmittable() || $job->isContinuous()) {
      ...
      if (!$translators = tmgmt_translator_labels_flagged($job)) {
        $this->messenger()->addWarning(t('There are no providers available. Before you can checkout you need to @configure at least one provider.', array('@configure' => Link::fromTextAndUrl(t('configure'), Url::fromRoute('entity.tmgmt_translator.collection'))->toString())));
      }
